...
const wchar_t* DiaDecl::GetName()
{
    if ( mName == NULL )
    {
        ...
        nChars = MultiByteToWideChar( CP_UTF8, MB_ERR_INVALID_CHARS, pstrName->name, pstrName->len, NULL, 0 );
        if ( nChars == 0 )
            return NULL;
        ...
        nChars = MultiByteToWideChar( CP_UTF8, MB_ERR_INVALID_CHARS, pstrName->name, pstrName->len, ccomBstrName.m_str, nChars + 1 );
        if ( nChars == 0 )
            return NULL;
